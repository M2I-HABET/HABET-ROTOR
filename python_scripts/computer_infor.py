...
class FT817(object):
    ...
    def __init__(self, serial_port, serial_speed=SERIAL_SPEED):
        self._serial = serial.Serial(serial_port, serial_speed)
        self._frequency = ""
        self._mode = ""
        self._squelch = True
        self._s_meter = ""
    ...
    def read_frequency(self):
        ...
        print(str(resp[0]))
        resp_bytes = (ord(str(resp[0])), 
            ord(str(resp[1])), 
            ord(str(resp[2])), 
            ord(str(resp[3])))
        self._frequency = "%02x%02x%02x%02x" % resp_bytes
        self._mode = FT817.MODES[ord(str(resp[4]))]
    ...
    def read_rx_status(self):
        ...
        '''
        for cmd in FT817.CMD_READ_RX_STATUS:
            self._serial.write(cmd)
